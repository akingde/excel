...
import com.example.demo.utils.excel.read.ExcelReadFieldMapping.ExcelReadFieldMappingAttribute;
import com.example.demo.utils.excel.write.*;
import com.example.demo.utils.excel.write.ExcelWriteFieldMapping.ExcelWriteFieldMappingAttribute;
...
public class ExcelUtils {
    ...
    private static void convertFieldMapping(Sheet sheet, ExcelReadSheetProcessor<?> sheetProcessor,
        ...
        for (Entry<String, Map<String, ExcelReadFieldMappingAttribute>> entry : src.entrySet()) {
    ...
    private static void readConfigParamVerify(ExcelReadSheetProcessor<?> sheetProcessor,
        ...
        for (Entry<Integer, Map<String, ExcelReadFieldMappingAttribute>> indexFieldMapping : fieldMapping.entrySet()) {
            for (Entry<String, ExcelReadFieldMappingAttribute> filedMapping : indexFieldMapping.getValue().entrySet()) {
                    ...
                    if (pageSize != null) {
                        ...
                        for (int i = 0; i < pageCount; i++) {
                            ...
                            read(controller, context, sheet, start, size, fieldMapping, clazz,
                                 sheetProcessor.getRowProcessor(), sheetProcessor.isTrimSpace(), sheetProcessor.isDenyAll());
                            if(!sheetProcessor.isDenyAll() || !context.isHasError()){
    ...
    private static <T> void read(ExcelProcessControllerImpl controller, ExcelReadContext<T> context, Sheet sheet,
                                 ...
                                 Map<Integer, Map<String, ExcelReadFieldMappingAttribute>> fieldMapping,
                                 Class<T> targetClass, ExcelReadRowProcessor<T> processor, boolean isTrimSpace, boolean isDenyAll) {
        ...
        for (int i = startRow; i <= endRow; i++) {
            if(context.getFailRowCount()>=ExcelReadContext.MAX_ERROR_NUM){
                return ;
            }
            ...
            if (isDenyAll && context.isHasError()) {
                context.incFailRowCount();
                break;
            }

            if (!controller.isDoSkip()) {
                context.incSuccessRowCount();
                list.add(t);
            }
            if (controller.isDoBreak()) {
                break;
    ...
    private static <T> T readRow(ExcelReadContext<T> context, Row row,
        ...
        for (Entry<Integer, Map<String, ExcelReadFieldMappingAttribute>> fieldMappingEntry : fieldMapping.entrySet()) {
            ...
            for (Entry<String, ExcelReadFieldMappingAttribute> fieldEntry : fields.entrySet()) {
                ...
                //自定义的校验
                if(attribute.getCellCheck() != null){
                    ExcelMessage excelMessage = new ExcelMessage();
                    boolean check = attribute.getCellCheck().check(context, new ExcelCellValue(value), excelMessage);
                    //校验不通过
                    if(!check){
                        hasError= true;
                        excelMessage.setRowIndex(curRowIndex);
                        excelMessage.setColIndex(curColIndex);
                        excelMessage.setSheetName(context.getCurSheetName());
                        if(excelMessage.getMessage() == null){
                            excelMessage.setMessage("未知错误信息");
                        }
                        context.getErrorMessageList().add(excelMessage);
                        continue;
                    }
                }
    ...
    private static void writeHead(boolean useTemplate, Sheet sheet, ExcelWriteSheetProcessor sheetProcessor) {
        ...
        for (Entry<String, Map<Integer, ExcelWriteFieldMappingAttribute>> entry : sheetProcessor.getFieldMapping().export().entrySet()) {
            Map<Integer, ExcelWriteFieldMappingAttribute> map = entry.getValue();
            if (map != null) {
                for (Entry<Integer, ExcelWriteFieldMappingAttribute> entry2 : map.entrySet()) {
    ...
    private static void writeStyleAfterFinish(boolean useTemplate, Sheet sheet,
        ...
        for (Entry<String, Map<Integer, ExcelWriteFieldMappingAttribute>> entry : mme.entrySet()) {
    ...
    private static void writeRow(ExcelWriteContext context, InnerRow templateRow, Row row, Object rowData,
        ...
        for (Entry<String, Map<Integer, ExcelWriteFieldMappingAttribute>> entry : fieldMapping.export().entrySet()) {
            ...
            Map<Integer, ExcelWriteFieldMappingAttribute> map = entry.getValue();
            for (Entry<Integer, ExcelWriteFieldMappingAttribute> fieldValueMapping : map.entrySet()) {
     ...
     *     1  -> 'B'
     *     28 -> 'AC'
     * @param index column integer index.
